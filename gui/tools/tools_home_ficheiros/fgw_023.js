define(['angularAMD', 'uxfwk'
        , 'uxfwk.require.lang!fgw.router.common'
...
return ['$scope', 'appUtilities', function controller ($scope, appUtilities){
   ...
   common.linkStatus = {
         ...
         disconnected:           3,
         connecting:             4,
         connected:              5, // Apresentar IPv4 e IPv6 do objecto WAN
      p:null};// linkStatus
function vshack(){ vshack.catch(); };// SlickEdit hack, do not remove!!!
...
(function _initialize (){
   ...
   if ( !$scope[key].criticalError ){
      ...
      deps.push('modules/{0}.router/{0}.router.dao'.sprintf(solution));
      //deps.push('modules/{0}.home/{0}.home.common'.sprintf(solution));
      require(deps, function(){

         // Inject dependencies
         dao  = $injector.get('uxfwk.{0}.router.dao'.sprintf(solution));
